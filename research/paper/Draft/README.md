...
experimental result of citation network. LINE, by Tang et al., employs second order proximity
...
performance in citation graphs. From this evidence, we hypothesize that a graph embedding
quality can be improved by using statistically significant sub-graphs within the given graph.
In the most recent work in graph embedding, Grover et al. proposed Node2vec algorithm [Node2vec],
which controls the graph context generation by a biased random walk. This proposed biased random
walk idea is similar to our graph context generation in this paper. While both node2vec and our algorithms
perform biasd random walk, the objectives are different. Node2vec aims to emphasize ``important''
...
the learning procedure. Planetoid is empirically proven to outperform previous algorithms. 
Albeit having good graph embedding performance, we consider Planetoid belongs to another
category since the algorithm requires ground-truth graph labels input. The discussion of
combining Planetoid framework and our embedding algorithm is in Section 5 of this paper.
