...
is a Monte Carlo chain [MonteCarlo], whose number of states equals the motif size. At each 
step along the motif walk, an adjacent vertex is picked by the rejection sampling procedure,
the rejection probability depends on the state of the Monte Carlo chain. The general algorithm
