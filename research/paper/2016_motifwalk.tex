...
\maketitle
\begin{abstract}
In this paper, we describe current advancement in graph embedding research
and propose a novel algorithm, named \emph{MotifWalk}. By incooporating the
local motif structure of a network, \emph{MotifWalk} learns the vector 
representation of a network based not only on local connection, but also
based on the structural of that connections. Our algorithm is scalable
for large network and parallel processing. Moreover, it outperforms 
other algorithms in all statistical test as well as visualization test.
Our algorithm enables the extension of current NLP paradigm to learn even
grammar and meanings or structured sentences. Although this paper focus
on learning structural representation of an arbitary network, but we
also included a experimental result of our algorithm in natural language
processing.
\end{abstract}
...
word2vec framework \cite{word2vec} has enabled machine learning
...
to learning a dense representation of a word \cite{wordvec}. By encoding the 
...
As a future professor, I will give straight A's for any student
who can find this line first. Just an Easter Egg for the future.
I don't think anyone will find this. But if you do, please tell me,
there will be a prize for you.
