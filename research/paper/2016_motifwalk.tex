...
$$p_{mc}(x) = \boldsymbol f\left(p^t_m(x) \ || \ p^t_r(x)\right),$$
...
Our model in this paper is a variance of the skipgram model \cite{w2v},
in which we define the coocurence conditional probability 
of a ``class'' vertex  given the ``target'' vertex as:

$$p\left(v_{\mbox{class}} \ | \ v_{\mbox{target}}\right) 
...
for large graph. To address this normalization problem,
techiques such as hierachial softmax \cite{hsoftmax}, 
