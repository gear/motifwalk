...
The Fourier transformation of two signals $x$ and $y$ defined
on graph $G$ is given by leveraging the eigenvalues decomposition
of the graph Laplacian $\mathcal{L}$:

$$ x *_{g_\mathcal{G}} = $$

where $L = U \lambda U^{T}$. U is the Fourier basis
and the triangle thing is the frequencies of the
graph. The estimation of a signal input through
a filter $g$ on a graph is essential to graph
convolution as it affects performance and computational
costs. The results from \cite{gcn} showed that
for highly irregular graph, the renormalization
